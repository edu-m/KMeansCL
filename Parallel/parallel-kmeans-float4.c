...
void test_print_dataset(cl_float4 *dataset, int points) {
  for (int i = 0; i < points / 2; ++i) {
    cl_float2 p0 = dataset[i].lo;
    cl_float2 p1 = dataset[i].hi;
    printf("[%d] X: %f Y: %f\n", i * 2, p0.x, p0.y);
    printf("[%d] X: %f Y: %f\n", i * 2 + 1, p1.x, p1.y);
  }
}
...
cl_event update_centroids(cl_command_queue que, cl_kernel centroids_kernel,
                          cl_mem *dataset, cl_mem *cluster_sum,
                          cl_mem *assignments, cl_mem *cluster_elements,
...
int main(int argc, char **argv) {
  ...
  cl_program prog = create_program("kmeans_f4.ocl", ctx, d);
  ...
  curr_row = 0;
  cl_float4 *dataset = malloc(points / 2 * sizeof(cl_float4));
  ...
  */
  ...
  cl_mem d_dataset =
      clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                     points / 2 * sizeof(cl_float4), dataset, &err);
  ...
  //                     centroids, 1, &assign_centroids_evt, NULL);
  // clFinish(que);
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    update_centroids_evt =
        update_centroids(que, update_centroids_kernel, &d_dataset,
                         &d_cluster_sum, &d_assignments, &d_cluster_elements,
  ...
  for (int i = 0; i < points / 2; i++) {
    ...
    printf("point %d (%f, %f) is in cluster %d (with centroid (%f,%f))\npoint "
           "%d (%f, %f) is in cluster %d (with centroid (%f,%f))\n",
           i * 2, p0.x, p0.y, assignments[i], centroids[assignments[i]].x,
           centroids[assignments[i]].y,

           i * 2 + 1, p1.x, p1.y, assignments[i + 1],
           centroids[assignments[i + 1]].x, centroids[assignments[i + 1]].y);
  }
  for (int i = 0; i < k; ++i)
